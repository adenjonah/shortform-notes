...
- `--vision agentic` (claude-code/codex only) writes the sampled frames to a temp dir for the call and grants `claude -p` exactly one tool: `--tools Read` sets the *available* tool set (not just the permitted one), `--allowed-tools Read` keeps a `-p` run from stalling on a prompt it cannot answer, and `--add-dir` scopes it to that directory. Verified against the installed CLI: the run reports Read as its only tool and cannot write or shell out.
