...
#   ./start.sh            → installs uv (if missing), Python + deps, opens the setup UI
#   ./start.sh <url>      → same setup, then imports the link from the terminal instead
set -euo pipefail
cd "$(dirname "$0")"
...
echo "▸ Installing Python 3.12 + reelnotes dependencies (first run takes a minute)…"
